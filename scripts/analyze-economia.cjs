...
const dir = path.join(__dirname, '..', '1- Poblacion', '7- Características económicas');
...
const outPath = path.join(__dirname, 'economia-output.txt');
fs.writeFileSync(outPath, out.join('\n'), 'utf8');
console.log(`Done! ${out.length} lines -> ${outPath}`);
console.log(`Sheets: ${files.map(f => XLSX.readFile(path.join(dir, f)).SheetNames.join(', ')).join(' | ')}`);
