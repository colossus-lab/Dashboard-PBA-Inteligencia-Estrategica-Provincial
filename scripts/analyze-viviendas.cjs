...
const dir = path.join(__dirname, '..', '1- Poblacion', '5- Viviendas');
...
for (const file of files) {
  ...
  out.push(`\n${'═'.repeat(80)}`);
  out.push(`FILE: ${file} (${sizeKB}KB) | Sheets: ${wb.SheetNames.join(', ')}`);
  out.push(`${'═'.repeat(80)}\n`);
  ...
  for (const sn of wb.SheetNames) {
    ...
    for (let i = 0; i < data.length; i++) {
      const row = data[i];
      const cells = row.map((c, j) => `[${j}]${String(c).trim()}`).filter(c => c !== `[${row.indexOf('')}]`);
      const nonEmpty = row.map((c, j) => `[${j}]${String(c).trim()}`).filter(c => !c.endsWith(']'));
      if (nonEmpty.length > 0) {
        out.push(`    R${String(i).padStart(2,'0')}: ${nonEmpty.join(' | ')}`);
      }
    }
    out.push('');
  }
...
const outPath = path.join(__dirname, 'viviendas-output.txt');
fs.writeFileSync(outPath, out.join('\n'), 'utf8');
console.log(`Done! ${out.length} lines written to ${outPath}`);
console.log(`Files processed: ${files.join(', ')}`);
